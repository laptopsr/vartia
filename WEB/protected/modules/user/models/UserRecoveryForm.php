...
class UserRecoveryForm extends CFormModel {
	...
	{
		return array(
			'login_or_email'=>UserModule::t("Käyttäjätunnus tai sähköposti osoite"),
		);
	}
	...
	public function checkexists($attribute,$params) {
		if(!$this->hasErrors())  // we only want to authenticate when no input errors
		{
			...
			if($user===null)
				if (strpos($this->login_or_email,"@")) {
					$this->addError("login_or_email",UserModule::t("Sähköposti on virheellinen."));
				} else {
					$this->addError("login_or_email",UserModule::t("Käyttäjätunnus on virheellinen."));
				}
		}
	}
