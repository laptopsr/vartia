...
class UserChangePassword extends CFormModel {
	...
	{
		return array(
			'oldPassword'=>UserModule::t("Vanha salasana"),
			'password'=>UserModule::t("Uusi salasana"),
			'verifyPassword'=>UserModule::t("Vahvista uusi salasana"),
		);
	}
	 ...
	 */
	 public function verifyOldPassword($attribute, $params)
	 {
		 if (User::model()->notsafe()->findByPk(Yii::app()->user->id)->password != Yii::app()->getModule('user')->encrypting($this->$attribute))
			 $this->addError($attribute, UserModule::t("Vanha salasana on virheellinen."));
	 }
}
